...
def initialize(title, description): # lav den her funktion i python i stedet for.
    inputs = []
    outputs = ["exists.txt"] # Is a directory an output? Update: no.
...
def bwa_map_pe(title, refgenome, read1, read2, subject):
    ...
    options = {'cores': 16, 'memory': 8000, 'walltime': '24:00:00'} # changed memory to 16 # more time for clint
    spec = '''
...
def merge_bams_new(title, subject, infiles, outfile, input):
    ...
    spec = '''
        ...
        #/home/cmkobel/sambamba-0.7.0-linux-static merge -t 4 {subject}/{outfile} {inbams}
        # Didn't help
        
        #trying picard
        /home/cmkobel/software/gatk-4.1.2.0/gatk MergeSamFiles {inbams} -O {subject}/{outfile}
        ...
        echo "All done at "$(date) > {subject}.COMPLETED'''.format(
            title = title,
            outfile  = outfile,
            inbams = ' '.join(['-I ' + str(i) for i in infiles]),
            subject = subject)
...
def filter_bam_file(title, individual):
    ...
    options = {'cores': 1, 'memory': 8000, 'walltime': '04:00:00'}
    spec = '''
        cd {title}
        source /com/extra/sambamba/0.5.1/load.sh
